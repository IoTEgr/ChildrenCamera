	...
	switch(timer)
	{
		case TIMER0 :
			...
			if(percent != 1)
				...
				R_TIMER_CTRL0 &= ~BIT(1); //TIMER disable and work in PWM	
			break;
		case TIMER1 : 
		...
		case TIMER2 : 
			...
			if(percent != 1)
				...
				R_TIMER_CTRL2 &= ~BIT(1); //TIMER disable and work in PWM	
			break;
		case TIMER3 : 
...
s8  ax32xx_timerPWMStop(u8 timer)
{
	...
	switch(timer)
	{
		case TIMER0 : 
			  ...
			  ax32xx_gpioSFRSet(GPIO_MAP_TMR0,TMR0_POS_NONE);
			  break;
		case TIMER1 : 
			  R_TIMER_CTRL1   = 0;
			  ax32xx_sysClkSet(SYS_CLK_TIMER1,0);
			  ax32xx_gpioSFRSet(GPIO_MAP_TMR1,TMR1_POS_NONE);
			  break;
		case TIMER2 : 
			  R_TIMER_CTRL2   = 0;
			  ax32xx_sysClkSet(SYS_CLK_TIMER2,0);
			  ax32xx_gpioSFRSet(GPIO_MAP_TMR2,TMR2_POS_NONE);
			  break;
		case TIMER3 : 
			  R_TIMER_CTRL3   = 0;
			  ax32xx_sysClkSet(SYS_CLK_TIMER3,0);
			  ax32xx_gpioSFRSet(GPIO_MAP_TMR3,TMR3_POS_NONE);
			  break;
		default : return -1;break;		
	}
}
