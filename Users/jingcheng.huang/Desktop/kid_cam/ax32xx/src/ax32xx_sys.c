...
void ax32xx_sysInit(u32 *saddr, u32 *eaddr)
{
    debg("sys init\n");
    // 中断全局禁用（保证初始化原子性）
    __LGIE_DIS__(); // 低优先级中断禁用
    __HGIE_DIS__(); // 高优先级中断禁用

    ax32xx_wdtClear(); // 看门狗复位

    //-----cache set
    ax32xx_sysIcacheInit(); // 指令缓存初始化

    ax32xx_sysDcacheInit(); // 数据缓存初始化
    //-----bss clear
    ax32xx_sysBSSClear(); // 清零BSS段（未初始化全局变量区）
    ...
    ax32xx_sysLDOSet(SYS_LDO_V1_2, SYS_VOL_V1_3, 1); // VDDCORE is better to config before clktun// 设置内核电压1.3V
    ax32xx_sysCpuMsDelay(20);
    ax32xx_sysLDOSet(SYS_LDO_LSEN, SYS_VOL_V3_0, 1); // 启用低压传感器
    ax32xx_sysCpuMsDelay(20);
    ax32xx_sysLDOSet(SYS_LDO_HSEN, SYS_VOL_V3_0, 1); // 启用高压传感器

    //-----system colck
    ax32xx_sysClkSet(SYS_CLK_DVP_CSI, 1); // 摄像头接口时钟
    ax32xx_sysClkSet(SYS_CLK_JPGA, 1);    // JPEG编码器A时钟
    ax32xx_sysClkSet(SYS_CLK_JPGB, 1);    // JPEG编码器B时钟
    ax32xx_sysClkSet(SYS_CLK_RTC, 1);     // 实时时钟
    // ax32xx_usbPhyPllEnable();
    fp_usb_isr = ax32xx_usbIRQHanlder; // 注册USB中断处理函数

    //----INT initial
    ax32xx_intInit(); // 初始化中断控制器
    if (saddr)
    ...
    ax32xx_dmaNocDefault(); // 设置DMA默认路由规则

    ax32xx_wdtEnable(1); // 启用看门狗定时器

    __LGIE_EN__(); // 启用低优先级中断
    __HGIE_EN__(); // 启用高优先级中断
}
/*******************************************************************************
