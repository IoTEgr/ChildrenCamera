	...
	switch(msg->id)
	{
		...
		case MSG_WIN_PAINT:
			if(!dirction){
				...
				length=pWin->rect.y1-pWin->rect.y0+1;
				length=pprogressBar->percentage*length/100;
				if(length==0)
					drawRectangle(&(pWin->rect),pWin->bgColor);
				else if(pprogressBar->percentage==100)
					...
					rect.y0=pWin->rect.y1-length+1;
					rect.y1=pWin->rect.y1;
					rect.x0=pWin->rect.x0;
					rect.x1=pWin->rect.x1;//rect.x0+length-1;
					drawRectangle(&rect,pprogressBar->color);
					rect.y0=pWin->rect.y0;
					rect.y1=pWin->rect.y1-length+1;
					drawRectangle(&rect,pWin->bgColor);
		...
		case MSG_WIN_PROGRESS_RATE:
			...
			winUpdate(hWin);
			return;
		case MSG_WIN_PROGRESS_DIRECTION:
			dirction = 1;
			break;
...
void progressBarSetDirction(uint32 dir)
{
	dirction =dir;
}
