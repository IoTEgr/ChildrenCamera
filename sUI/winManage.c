...
void winDestroy(winHandle *hWin)
{
	...
	// 获取窗口对象和父窗口信息
	pWin = (winObj *)uiHandleToPtr(*hWin);
	invalidRect.x0 = pWin->rect.x0; // 记录原始窗口区域
	invalidRect.x1 = pWin->rect.x1;
	invalidRect.y0 = pWin->rect.y0;
	invalidRect.y1 = pWin->rect.y1;
	hParent = pWin->parent; // 保存父窗口句柄
	...
	else
	{
		...
		state.destroyWin = 1; // 设置销毁标志位

		// 触发窗口销毁消息链
		winSendMsg(*hWin, &msg);
		state.curWin = hParent; // 切换当前窗口到父窗口
		state.destroyWin = 0;	// 清除销毁标志

		/*
		双缓冲切换问题说明：
		在录像模式关闭菜单时，由于子窗口关闭消息先于重绘消息发送，
		会导致从双缓冲切换回单缓冲时出现闪屏现象
		*/
		msg.id = MSG_WIN_INVALID;
		msg.parameter.p = (void *)&invalidRect;
		winSendMsg(hParent, &msg); // 通知父窗口重绘

		*hWin = INVALID_HANDLE;							  // 置空窗口句柄
		winSendMsgNoData(hParent, MSG_WIN_CHILE_DESTROY); // 子窗口销毁通知
		uiUnlock();
		return;
	}
...
int winTouch(uiRect *rect, uint32 touchState)
{
	...
	if (infor.touchWin == lastInfor.touchWin && infor.touchID == lastInfor.touchID && infor.touchItem == lastInfor.touchItem)
	{
		return 1;
	}
...
void winSetPorgressDirction(winHandle hWin, uint32 dirction)
{
	winMsg msg;
	msg.id = MSG_WIN_PROGRESS_DIRECTION;
	msg.parameter.v = dirction;
	winSendMsg(hWin, &msg);
}
...
void winInit(void)
{
#define WIN_HEAP_SIZE (1024 * 40) //(1024*24)//20220823
