...
void osd_conver(u8 *src,u8*dst)
{
	if(hal_lcdGetShowingRotate()==LCD_ROTATE_90) //90
	{
		//u8 *src = (u8 *)setting_frame_buff;
		s32 i, j;
		for(i = 0; i < OSD_HEIGHT; i++){
		for(j = OSD_WIDTH - 1; j >= 0; j--){
		*(dst + j*OSD_HEIGHT + i) = *(src++);
		}
		}
	}
	else   //270
...
void osdBuffCopy(u8* buff,u32 wide,u32 high)
{
	//return;
	/*u8* osdBuff=getDrawBuff();
	uint32 w,h;
	osdBuff=osdBuff+(LCD_WIDTH<<3)+112;
	for(h=0;h<high;h++)
	{
		for(w=0;w<wide;w++)
			osdBuff[w]=buff[w];
		//ax32xx_sysDcacheWback((u32)osdBuff,wide);
		osdBuff+=LCD_WIDTH;
		buff+=wide;
	}*/
	//ax32xx_deWait();
	...
	u8* dst = (void *)dispLayerGetFrame(DISP_LAYER_OSD0);
	u8* src = (void *)doubleBuffer;
	
	osd_conver(src,dst);
	ax32xx_sysDcacheWback((u32)dst,OSD_HEIGHT*OSD_WIDTH);
	ax32xx_deUpdate();
...
INT8U* gameGetDrawBuff(void)
{
	INT8U* addr = NULL;
	if(useDoubleBuffer){
		ax32xx_sysDcacheWback((u32)doubleBuffer,OSD_HEIGHT*OSD_WIDTH);
		return doubleBuffer;
	}
	addr = (INT8U*)dispLayerGetFrame(DISP_LAYER_OSD0);
	return addr;
}
