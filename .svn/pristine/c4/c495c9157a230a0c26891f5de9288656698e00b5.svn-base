...
{
    u16 csi_w,csi_h,crop_w,crop_h,tft_w,tft_h;
    u16 sx,sy,ex,ey;
	hal_lcdGetResolution(&tft_w,&tft_h);
	tft_w=(tft_w + 0x1f) & (~0x1f);  // add 32bit alignment
    hal_csiResolutionGet(&csi_w,&csi_h);
	if(640==csi_w)
	{
		
		crop_w=csi_w-level*11;
		crop_h=csi_h-level*8;
		if(crop_w < tft_w)
			crop_w = tft_w;
		if(crop_h < tft_h)
			crop_h = tft_h;
	...
	deg_Printf("crop:sx=%d,sy=%d,ex=%d,ey=%d\n",sx,sy,ex,ey);
