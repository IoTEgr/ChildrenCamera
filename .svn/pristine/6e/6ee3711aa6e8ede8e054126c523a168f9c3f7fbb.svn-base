...
#define  ICON_PALETTE_GAME_ADDR   RES_PALETTE_GAME        
...
int iconGetGamePalette(INT8U *buffer)
{
	int ret,size;

	ret = nv_open(ICON_PALETTE_GAME_ADDR);
	if(ret<0)
	{
		deg_Printf("icon : find palette fail\n");
		return -1;
	}
	size = nv_size(ICON_PALETTE_GAME_ADDR);
