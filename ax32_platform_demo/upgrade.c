...
//-------get cache memory
	...
	deg_Printf("cur ver:<%s>.\n",layout_version_get());
	//addr = 0x7ca50;
	addr = nv_open(RES_STR_VERSION);
	deg_Printf("addr=0x%x.\n",addr);
	lseek(fd,addr,0);
	ret = read(fd,buffer,FIRMWARE_LEN);
	deg_Printf("firmware:<%s>.\n",buffer);

	if( strcmp(layout_version_get(),buffer) == 0)
	{
		deg_Printf("Same version, no upgrade required.\n");	
		close(fd);
		if(buffer)
...
//-------erase flash
	...
	strHandle=stringIconCreate(R1x(20),R1y(125),R1w(280),R1h(35),INVALID_HANDLE,WIN_ABS_POS,0);
