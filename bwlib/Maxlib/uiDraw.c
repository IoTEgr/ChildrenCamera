...
#define  ICON_SPEC_NUM 		1

#define  ICON_BUFF_NUM         16
#define  ICON_BUFF_SIZE        (40*40)
...
{
//	PRELOAD_ICON(R_ID_ICON_MTRECORDING),
//	PRELOAD_ICON(R_ID_ICON__TIPS_BK),
};
...
void preloadIconInit(void)
{
	...
	if(loadIcon==true)
		return;
	loadIcon=true;
	for(i=0;i<(sizeof(preloadIcon)/sizeof(preloadIcon[0]))-ICON_SPEC_NUM;i++)
	{
		addr=resGetImageAddrAndSize(preloadIcon[i].id,&width,&height);
		if(addr==0)
		{
...
void preloadIcon_spec_Init(void)
{
	uint16 width,height,i;
	uint32 len;
	uint32 addr;

	for(i=(sizeof(preloadIcon)/sizeof(preloadIcon[0]))-ICON_SPEC_NUM;i<(sizeof(preloadIcon)/sizeof(preloadIcon[0]));i++)
	{
		addr=resGetImageAddrAndSize(preloadIcon[i].id,&width,&height);
		if(addr==0)
		{
...
void preloadIcon_spec_Uninit(void)
{
	u16 i;
	for(i=(sizeof(preloadIcon)/sizeof(preloadIcon[0]))-ICON_SPEC_NUM;i<(sizeof(preloadIcon)/sizeof(preloadIcon[0]));i++)
	{
		...
		{
			hal_sysMemFree(preloadIcon[i].buff);
			preloadIcon[i].buff=NULL;
			preloadIcon[i].addr=0;
			preloadIcon[i].width=0;
			preloadIcon[i].height=0;
			preloadIcon[i].count=0;
		}
	}
}
...
uint32 resGetStringInfor(resID id,uint16* width,uint16* height,charFont font)
{
	...
	if(resIdIsStr(id))
	{
		...
		if(str==NULL)
			return 0;
		if(0==SysCtrl.mp3_font_use)
		{
			return R_getAsciiStringSize(str,width,height,font);
		}
		else
		{
			return MP3_get_unicode_str_size(str,width,height,font);
		}
...
uint8* resGetCharInfor(resID id,uint8 num,uint16* width,uint16* height,charFont font,uint8* special)
{
	...
	if(resIdIsStr(id))
	{
		...
		{
			return MP3_unicode_draw_get(str,num,(u16 *)width,(u16 *)height,font);
		}
	}
	else if(ID_TYPE(id)==R_ID_TYPE_WAV_NAME)
	{
...
void drawRectangleToBuf(u8*drawBuff,u16 buf_w,u16 buf_h,uiRect* rect,uiColor color)
{
	...
	width=rect->x1-rect->x0+1;
	hight=rect->y1-rect->y0+1;
	drawBuff+=rect->y0*buf_w+rect->x0;
...
void drawRectangleToBuf(u8*drawBuff,u16 buf_w,u16 buf_h,uiRect* rect,uiColor color)
{
	...
	else if(lcdRotate==LCD_ROTATE_270)
	{
		width=rect->y1-rect->y0+1;
		hight=rect->x1-rect->x0+1;
		drawBuff+=rect->x0*buf_h+(buf_h-1-rect->y1);
	}
	else if(lcdRotate==LCD_ROTATE_180)
	{
		width=rect->x1-rect->x0+1;
		hight=rect->y1-rect->y0+1;
		drawBuff+=(buf_h-1-rect->y1)*buf_w+(buf_w-1-rect->x1);
	}
	else if(lcdRotate==LCD_ROTATE_90)
	{
		width=rect->y1-rect->y0+1;
		hight=rect->x1-rect->x0+1;
		drawBuff+=(buf_w-1-rect->x1)*buf_h+rect->y0;
	}
	else
		return;
