...
XMsgQ *XMsgQCreate(MSG_T *queue, INT8U len)
{
    ...
    for (i = 0; i < X_CFG_MSGQ_MAX; i++)
    {
        ...
        {
            XMsgQuePool[i].stat = len;  // 队列总容量（创建时设置，不可变）
            XMsgQuePool[i].iput = 0;    // 入队位置索引（环形缓冲区写指针）
            XMsgQuePool[i].oput = 0;    // 出队位置索引（环形缓冲区读指针）
            XMsgQuePool[i].len = 0;     // 当前队列中的消息数量
            XMsgQuePool[i].msg = queue; // 消息存储数组指针（用户提供内存）
            X_CRITIAL_EXIT();
