...
#define  HAL_VIDEO_RECORD_SHRINK_EN     1 // Recording time contraction recording function add by henry 2021.06.17
...
void videoRecordImageWatermark(INT16U width,INT16U height,INT8U en)
{
    ...
    if(en)
    {
		...
		hal_watermarkPosition(mediaVideoCtrl.tinfo,16,height-(value>>8)-inv);
...
int videoRecordCmdSet(INT32U cmd,INT32U para)
{
	...
	{
		mediaVideoCtrl.arg.fps  = para;
	}
	else if(cmd == CMD_COM_VIDEO_FAST_EN)
	{
		mediaVideoCtrl.arg.videorec_fast_flag = para;
	}
	else
		return STATUS_FAIL;
