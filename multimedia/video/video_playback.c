...
*/
			...
			memset(videoPCtrl.curframe+8,0,videoPCtrl.audiolen);
...
{   
	...
	videoPCtrl.volume =SysCtrl.volume_level*20;
...
int videoPlaybackStart(VIDEO_PARG_T *arg)
{
	...
	if(ret<0)
	{
		videop_print("video player : avi decode parse fail:%d\n",ret);
		return STATUS_FAIL;
	}
	...
	{
		deg_Printf("video player : malloc memory fail.%x,%x\n",(INT32U)videoPCtrl.jpegcache,(INT32U)videoPCtrl.audiobuffer);
		hal_sysMemFree(videoPCtrl.jpegcache);
		hal_sysMemFree(videoPCtrl.audiobuffer);
		return STATUS_FAIL;
	}
	deg_Printf("video player : mem.vids:num=%d,size=0x%x\n",jpgnum,jpgcachesize);
    hal_sdLock();
...
//-------cache jpeg data
        ...
        if(err == X_ERR_NONE)
        {
			...
			//deg_Printf("ret=%d,os=%x,len=%x,oldf=%x,flag=%d\n",ret,offset,length+8,videoPCtrl.oldframe,videoPCtrl.vflag);
...
/*
				...
				if(err_flag)
				{
					...
					memcpy(name,"001.jpg",8);
					name[0] = (index/100)%10+'0';
					name[1] = (index/10)%10+'0';
					name[2] = index%10+'0';
					fd = open((const char *)name,FA_READ|FA_WRITE|FA_CREATE_NEW);
					write((FHANDLE)fd,(void *)jpeg->mem+8,length);
    				close((FHANDLE)fd); 
					index++;
					err_flag = 0;
				}
    ...
    if(videoPCtrl.vflag)
    {
		...
		if(err == X_ERR_NONE)
		{
			...
			if(first_frame_not_display)
				dispLayerFreeFrame(DISP_LAYER_VIDEO,(disp_frame_t *)video->mem);
			else
...
*/
	...
	int fast;
	if(4!=videoPCtrl.playSpeed)
	{
		videoPCtrl.playSpeed=4;
	}
	else
	{
		videoPCtrl.playSpeed=5;
	}
	fast = videoSpeedTable[videoPCtrl.playSpeed];
	aviDecodeFast(fast);
